...
def _cut(sentence, max_seq_length):
    new_sentences = []
    sentences = re.split('([。|！|!|？|?])', sentence)
    for sen_one in sentences:
        if len(sen_one) > max_seq_length:
            sen_ones = re.split('[：|;]', sen_one)
            for sen_two in sen_ones:
                if len(sen_two) > max_seq_length:
                    sen_twos = re.split('[，|,]', sen_two)
                    for sen_three in sen_twos:
                        if len(sen_three) > max_seq_length:
                            sen_threes = re.split('[：|:]', sen_three)
                            for sen_five in sen_threes:
                                new_sentences.append(sen_five)
                        else:
                            new_sentences.append(sen_three)
                else:
                    new_sentences.append(sen_two)

        else:
            new_sentences.append(sen_one)
    sentences = [new_sentences[i] + new_sentences[i + 1] for i in range(0, len(new_sentences) // 2 * 2, 2)]
    if len(new_sentences) % 2:
        sentences.append(new_sentences[-1])
    return sentences

def cut_sentence(sentence, max_seq_length):
    context = []
    if len(sentence) != 0:
        # 判断这一句是否超过最大长度
        if len(sentence) > max_seq_length:
            sentence = _cut(sentence, max_seq_length)
...
def extract_one_sample(files):
    ...
    DrugPproperties = DocxSplitJson(file_name)
    # print(f"DrugPproperties = {DrugPproperties}")
    ...
    with graph.as_default():
        ...
        DRUGNAME = DrugPproperties['首行药品名称']
        node = {}
        ...
        # 用法用量
        usage_list = []
        usage_list.append('用法用量') if '用法用量' in DrugPproperties else ''
        usage_list.append('用法与用量') if '用法与用量' in DrugPproperties else ''
        usage_list.append('用量用法') if '用量用法' in DrugPproperties else ''
        usage_list.append('用量与用法') if '用量与用法' in DrugPproperties else ''
        usage_list.append('免疫程序和剂量') if '免疫程序和剂量' in DrugPproperties else ''
        usage_lines = ''
        for usage in usage_list:
            usage_lines += DrugPproperties[usage]
        if usage_lines:
            alter_sign = True
            sentence = usage_lines.replace("\n", "")
            sentence = sentence.replace(" ", "")

            # 先序列截断预测年龄
            sentence_list = []
            max_seq_length = args.max_seq_length
            sentence_list = cut_sentence(sentence, max_seq_length)
        ...
        # 禁忌 注意事项
        taboo_list = []
        taboo_list.append('注意事项') if '注意事项' in DrugPproperties else ''
        taboo_list.append('禁忌') if '禁忌' in DrugPproperties else ''
        taboo_list.append('禁忌症') if '禁忌症' in DrugPproperties else ''
        taboo_lines = ''
        for taboo in taboo_list:
            taboo_lines += DrugPproperties[taboo]
        if taboo_lines:
            reader = taboo_lines.replace("\n", "")
            # 生化指标.肝功能检查
        ...
        # 接种对象
        object_list = []
        object_list.append('接种对象') if '接种对象' in DrugPproperties else ''
        object_lines = ''
        for object in object_list:
            object_lines += DrugPproperties[object]
        if object_lines:
            reader = object_lines.replace("\n", "")
            if reader.find("男") != -1:
                node["病人资料.性别"] = "属于 男"
                # print(node["sex"][0])
        ...
        # 儿童用药
        chil_list = []
        chil_list.append('儿童用药') if '儿童用药' in DrugPproperties else ''
        chil_lines = ''
        for chil in chil_list:
            chil_lines += DrugPproperties[chil]
        if chil_lines:
            reader = chil_lines.replace("\n", "")
        ...
        # 老人用药
        old_list = []
        old_list.append('老年用药') if '老年用药' in DrugPproperties else ''
        old_lines = ''
        for old in old_list:
            old_lines += DrugPproperties[old]
        if old_lines:
            reader = old_lines.replace("\n", "")
        ...
        # 孕妇及哺乳期妇女用药
        girl_list = []
        girl_list.append('孕妇及哺乳期妇女用药') if '孕妇及哺乳期妇女用药' in DrugPproperties else ''
        girl_lines = ''
        for girl in girl_list:
            girl_lines += DrugPproperties[girl]
        if girl_lines:
            reader = girl_lines.replace("\n", "")
            if reader.find("孕妇") != -1:
                node["病人资料.是否怀孕"] = "= True"
...
def search_age(sentence, tokens, tags):
    ...
    buff = re.split(f"(新生儿|婴儿|儿童|幼儿|老人|老年人|高龄|早产儿|成人|小儿{split_str})", sentence)
    phase_dict_original = {}
    ...
    for key in phase_dict_original:
        if key in age_split_record.keys():
            ...
            age_field.append("小儿")
            phase_dict_final[">= 1岁 且< 3岁"] = phase_dict_original["小儿"]
        else:
...
def create_directory():
    if (not os.path.exists(f"{absp}/data/instructions")):
        ...
        print(f"{absp}/data/instructions")
...
def DocxSplitJson(file_name):
    ...
    compiled_rule_2 = re.compile(PATTERN_2)
    PATTERN_3 = r"M(\d+)"
    compiled_rule_3 = re.compile(PATTERN_3)
    DrugPproperties = {}
    document = Document(f'{path_instructions}/{file_name}.docx')
    # 读取文档中所有的段落列表
    ps = document.paragraphs
    ...
    for line in ps_detail:
        ...
        if (line == '' or line == '【】'):
            continue
        mm_1 = re.match(compiled_rule_1, line)  # 第一个字符开始匹配
        mm_2 = re.match(compiled_rule_2, line)
        if mm_1:
            find_name = True if mm_1.group(1) == "药品名称" or mm_1.group(1) == "通用名称" or mm_1.group(
                1) == "饮片名" or mm_1.group(1) == "产品名称" else False
            now_property = mm_1.group(1)
            DrugPproperties[f'{last_property}'] = content
            content = ''
            last_property = now_property
        elif mm_2:
            find_name = True if mm_2.group(1) == "药品名称" or mm_2.group(1) == "通用名称" or mm_2.group(
                1) == "饮片名" or mm_2.group(1) == "产品名称" else False
            now_property = mm_2.group(1)
            DrugPproperties[f'{last_property}'] = content
            content = ''
            last_property = now_property
        else:
            ...
            if find_name:
                ...
                if buf.find("通用名称") != -1:
                    ...
                    buf = buf.replace("：", "")
                    drug_name = buf.split("通用名称")[1]
                    DrugPproperties['首行药品名称'] = drug_name
                    find_sign = True
                else:
                    if not find_sign:
                        DrugPproperties['首行药品名称'] = buf
                        find_sign = True
    if not find_sign:
        DrugPproperties['首行药品名称'] = file_name
        mm_3 = re.match(compiled_rule_3, file_name)
        if mm_3 and DrugPproperties['首行']:
            name_list = DrugPproperties['首行'].split('：')
            if len(name_list) > 1:
                DrugPproperties['首行药品名称'] = name_list[1]
    # print(f'DrugPropertoes = {json.dumps(DrugPproperties, indent=4, ensure_ascii=False)}')
    # print(f"药品名称 = {DrugPproperties['首行药品名称']}")
    return DrugPproperties
