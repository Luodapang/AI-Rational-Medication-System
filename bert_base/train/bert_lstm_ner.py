...
class NerProcessor(DataProcessor):
    ...
    def read_data_field_num(self, data_dir, output_dir):
        """Reads a BIO data."""
        list_txt = ["train", "test", "dev"]
        for txt_name in list_txt:
            with codecs.open(os.path.join(data_dir, f"{txt_name}.txt"), 'r', encoding='utf-8') as f:
                words = []
                labels = {"B-dosingFrequence": 0, "B-medicationInterva": 0, "B-useTreatment": 0, "B-eachTimeDose": 0,
                          "B-doseKGEachTime": 0, "B-maximumDoseKGEachTime": 0, "B-dayDose": 0, "B-dosingPurpose": 0,
                          "B-doseKGEachDay": 0}
                for line in f:
                    contends = line.strip()
                    tokens = contends.split(' ')
                    if len(tokens) == 2:
                        ...
                        if tokens[1] in labels:
                            labels[tokens[1]] = int(labels[tokens[1]]) + 1
                with codecs.open(os.path.join(output_dir, f"num_{txt_name}_field.txt"), 'w', encoding='utf-8') as w:
                    for key in labels.keys():
                        w.write(key + "\t" + str(labels[key]) + "\n")
    ...
    def get_labels(self, labels=None):
        ...
        if os.path.exists(os.path.join(self.output_dir, 'label_list.pkl')):
            ...
            if len(self.labels) > 0:
                ...
                self.labels = ["O", 'B-dosingFrequence', 'I-dosingFrequence', 'B-medicationInterva', 'I-medicationInterva', 'B-useTreatment'
                                , 'I-useTreatment', 'B-dayDose', 'I-dayDose', 'B-dosingPurpose', 'I-dosingPurpose', 'B-doseKGEachDay', 'I-doseKGEachDay'
                                , "X", "[CLS]", "[SEP]"]
        #         给药频率、间隔周期、使用疗程、每天剂量、给药目的、每公斤每日剂量
        return self.labels
                    ...
                    if len(contends) == 0 and len(words) > 0:
                        ...
                        for l, w in zip(labels, words):
                            if len(l) > 0 and len(w) > 0:
                                ...
                                self.labels.add(l)
                                word.append(w)
                        lines.append([' '.join(label), ' '.join(word)])# 列表列表[[],[]]
                        words = []
                        labels = []
                        continue
                if contends.startswith("-DOCSTART-"):
...
def filed_based_convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        def create_int_feature(values):
            # tf.train.Features(feature = None) 这里的feature是以字典的形式存在，*key：要保存数据的名字 value：要保存的数据，但是格式必须符合tf.train.Feature实例要求。
...
def get_last_checkpoint(model_path):
    if not os.path.exists(os.path.join(model_path, 'checkpoint')):
        tf.compat.v1.logging.info('checkpoint file not exits:'.format(os.path.join(model_path, 'checkpoint')))
        return None
    last = None
...
def train(args):
    ...
    if args.do_train and args.do_eval:
        # 写入各数据集数量
        processor.read_data_field_num(args.data_dir, args.output_dir)
        # 加载训练数据
    ...
    # 模型的构造，返回的model_dn 是一个函数
    # model_dn其定义了模型，训练，评测方法，并且使用钩子参数，加载了BERT模型的参数进行了自己模型的参数初始化过程
    ...
    # if args.do_train:
        ...
        train_input_fn = file_based_input_fn_builder(
            ...
            drop_remainder=True)
        # 3.模型的训练
        print("***** start training_1 *****")
        print(num_train_steps)
        estimator.train(input_fn=train_input_fn, max_steps=num_train_steps)
        print("***** end training_1 *****")
        ...
        print("***** start training_2 *****")
        tf.estimator.train_and_evaluate(estimator, train_spec, eval_spec)
        print("***** end training_2 *****")
        ...
        def result_to_pair(writer):
            for predict_line, prediction in zip(predict_examples, result):
                ...
                if len(line_token) != len(label_token):
                    tf.compat.v1.logging.info(predict_line.text)
                    tf.compat.v1.logging.info(predict_line.label)
                    break
