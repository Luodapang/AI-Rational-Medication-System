...
def create_model(bert_config, is_training, input_ids, input_mask,
    ...
    # bert的最后一层是所有transformer结果的最后一维，其是一个三维向量维度是：[batch_size, seq_length,embedding_size]，可以类比的理解为我们使用tf.nn.embedding_lookup获取的结果。
...
# 所以在代码中通过在add_blstm_crf_layer函数中的crf_only参数进行控制我们训练的时候使用的是那种网络结构用于最后的fine-tuning.通过两种结构的训练结果对比，其实他们的最后结果相差不大，可以说基本是一样的，足见transformer的强大。
